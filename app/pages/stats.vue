...
import { isRunningStats, runStatsAnalysis, statsError, statsReport } from '~/composables/stats'
...
<template>
  <div flex="~ col gap-4" my4>
    ...
    <div v-if="isStaticConnection" p3 color-muted italic>
      Stats analysis needs a live dev server to run ESLint on-demand.
      Run <code rounded bg-code px1>eslint-config-inspector</code> locally to use this tab.
    </div>
    ...
    <template v-else>
      ...
      <template v-if="statsReport && !isRunningStats">
        <div flex="~ gap-x-4 gap-y-1 items-center wrap" text-sm>
          ...
          <button ml2 btn-action-sm @click="runStatsAnalysis()">
            <div i-ph-arrow-clockwise-duotone />
            Re-run
          </button>
        </div>
